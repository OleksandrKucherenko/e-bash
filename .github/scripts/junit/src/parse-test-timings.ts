...
async function main() {
    ...
    console.error(`📊 Parsing timing data with granularity: ${granularity}`);
    ...
    if (granularity === "file") {
        ...
        console.error(`✅ Extracted timing data for ${Object.keys(merged).length} spec files`);
        console.error(`📊 Total test time: ${totalTime.toFixed(2)}s`);
        ...
        console.error(`✅ Extracted timing data for ${exampleCount} examples in ${fileCount} spec files`);
        console.error(`📊 Total test time: ${totalTime.toFixed(2)}s`);
        console.error(`💾 Saved to: ${outputFile}`);
