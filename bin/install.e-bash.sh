...
## Read .ebashrc configuration file and set SCRIPTS_DIR if custom directory is configured
function read_ebashrc() {
  if [ -f "$EBASHRC_FILE" ]; then
    # Source the config file to get E_BASH_INSTALL_DIR
    # shellcheck source=/dev/null
    source "$EBASHRC_FILE"

    if [ -n "$E_BASH_INSTALL_DIR" ]; then
      SCRIPTS_DIR="$E_BASH_INSTALL_DIR"
      echo -e "${BLUE}Using custom installation directory from ${EBASHRC_FILE}: ${YELLOW}${SCRIPTS_DIR}${NC}" >&2
    fi
  fi
}

## Write .ebashrc configuration file with custom installation directory
function write_ebashrc() {
  local custom_dir="${1:-$SCRIPTS_DIR}"

  if [ "$custom_dir" != "$DEFAULT_SCRIPTS_DIR" ]; then
    echo -e "${BLUE}Creating ${EBASHRC_FILE} with custom directory: ${YELLOW}${custom_dir}${NC}" >&2

    cat > "$EBASHRC_FILE" <<EOF
# e-bash configuration
# This file is automatically generated by install.e-bash.sh
# Version: 1.0

# Custom installation directory for e-bash scripts
E_BASH_INSTALL_DIR="$custom_dir"
EOF

    echo -e "${GREEN}Created ${EBASHRC_FILE}${NC}" >&2
  fi
}
...
function repo_uninstall() {
  ...
  # Remove .ebashrc configuration file
  if [ -f "${EBASHRC_FILE}" ]; then
    if [ "$DRY_RUN" = true ]; then
      echo -e "${CYAN}dry run: rm -f ${EBASHRC_FILE}${NC}"
    else
      rm -f "${EBASHRC_FILE}"
      echo -e "${GREEN}Removed ${EBASHRC_FILE}${NC}"
    fi
    ((uninstall_steps++))
  fi
...
function install_scripts() {
  ...
  if [ "$GLOBAL" = true ]; then # Global Installation
    ...
    # Create .ebashrc if custom directory is used
    write_ebashrc "$SCRIPTS_DIR"
  ...
  for i in "${!args[@]}"; do
    ...
    if [[ "$key" == "--dry-run" ]]; then
      ...
      if [ -n "${args[$next_idx]}" ] && [[ "${args[$next_idx]}" != --* ]]; then
        SCRIPTS_DIR="${args[$next_idx]}"
        unset 'args[i]' 'args[next_idx]'
      else
        echo -e "${RED}Error: --directory requires a path argument${NC}" >&2
        print_usage $EXIT_NO
      fi
...
# Read .ebashrc configuration if it exists (unless --directory was explicitly provided)
# Check if SCRIPTS_DIR was modified by --directory flag
if [ "$SCRIPTS_DIR" == "$DEFAULT_SCRIPTS_DIR" ]; then
  read_ebashrc
fi
