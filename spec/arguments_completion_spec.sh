...
Describe 'args:completion /'
  ...
  Mock echo:Common
    # suppress logger output in completion tests
    :
  End
  ...
  Describe '_args:get:all_flags() /'
    ...
    It 'returns all flags excluding positional arguments'
      BeforeCall 'export DEBUG="" ARGS_DEFINITION="-h,--help -v,--verbose --output=::1 \$1,<command>=cmd:dummy:1"'
    ...
    It 'returns empty for positional-only definitions'
      BeforeCall 'export DEBUG="" ARGS_DEFINITION="\$1,<command>=cmd:dummy:1"'
  ...
  Describe '_args:get:value_flags() /'
    ...
    It 'identifies flags that expect values'
      BeforeCall 'export DEBUG="" ARGS_DEFINITION="-h,--help -o,--output=output::1 -f,--format=format:text:1"'
    ...
    It 'returns empty when no flags expect values'
      BeforeCall 'export DEBUG="" ARGS_DEFINITION="-h,--help -v,--verbose"'
  ...
  Describe '_args:get:description() /'
    ...
    It 'returns description for a registered flag'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-h,--help"
        parse:mapping
  ...
  Describe '_args:completion:bash /'
    ...
    It 'generates valid bash completion script with flags'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-h,--help -v,--verbose -o,--output=output::1"
        parse:mapping
      }
      BeforeCall 'setup'
    ...
    It 'includes value flags in the completion script'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-h,--help -o,--output=output::1"
        parse:mapping
      }
      BeforeCall 'setup'
  ...
  Describe '_args:completion:zsh /'
    ...
    It 'marks value flags with :value:_files'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-o,--output=output::1"
        parse:mapping
        args:d "-o" "Output file path."
      }
      BeforeCall 'setup'
  ...
  Describe 'args:completion integration /'
    ...
    It 'generates bash completion via args:completion function'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-h,--help --verbose -o,--output=output::1"
        parse:mapping
      }
      BeforeCall 'setup'
    ...
    It 'generates zsh completion via args:completion function'
      setup() {
        export DEBUG="" ARGS_DEFINITION="-h,--help --verbose"
        parse:mapping
        args:d "-h" "Show help."
  ...
  Describe 'args:completion:install /'
    ...
    It 'creates directory if it does not exist'
      ...
      The status should be success
      The stdout should include 'nonexistent-dir/completions/testcmd'
