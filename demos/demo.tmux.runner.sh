...
usage() {
  ...
  echo "    -L, --layout <layout> : Specify pane layout ('T' or 'E'). Default: 'T'."
...
mkfifo "$FIFO_STDOUT" || { echo "Failed to create STDOUT FIFO"; exit 1; }
mkfifo "$FIFO_STDERR" || { echo "Failed to create STDERR FIFO"; rm -f "$FIFO_STDOUT"; exit 1; }
