...
function demo:completion:install() {
  ...
  if [[ "${shell_name}" == "zsh" ]]; then
    ...
    echo "Test:"
    echo "  demos/completion/demo.curl.sh --<TAB>"
  else
...
if [[ "$help" == "1" ]]; then
  ...
  echo ""
  echo "Usage: demos/completion/demo.curl.sh [global flags] <command> [flags]"
  echo "       demos/completion/demo.curl.sh --completion-install [bash|zsh]"
  echo ""
  print:help
  exit 0
fi
...
cat <<'SAMPLES'
Samples:
  demos/completion/demo.curl.sh get --url https://example.test
  demos/completion/demo.curl.sh post -X POST -d '{"a":1}' --url https://example.test
  demos/completion/demo.curl.sh head -H 'Accept: */*' --url https://example.test

Completion data:
  demos/completion/demo.curl.sh --completion-data

Completion install:
  demos/completion/demo.curl.sh --completion-install zsh
  demos/completion/demo.curl.sh --completion-install bash
SAMPLES
