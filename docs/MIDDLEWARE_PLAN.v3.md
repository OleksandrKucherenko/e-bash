# Hooks Middleware v3 Implementation Plan
...
**Architecture:** Keep hooks:do as the single entry point. Each exec-mode implementation (hook function, registered function, exec script) is wrapped with a capture harness that writes a prefixed timeline into a unique global array. A per-hook middleware function receives the hook name, exit code, and capture array name, applies contract logic (optional), replays output, and returns the effective exit code. Sourced hooks bypass middleware.
...
```bash
# in hooks:do, change warning to info-style text
...
### Task 2: Add middleware registry + reset coverage

**Files:**
- Modify: `.scripts/_hooks.sh`
- Modify: `spec/hooks_spec.sh`

**Step 1: Write failing tests (register/reset)**
...
**Step 3: Implement registry + reset**
...
### Task 3: Add capture harness for exec-mode implementations
...
Add `source "$E_BASH/_commons.sh"` in `.scripts/_hooks.sh` to use `to:slug`.
...
### Task 4: Wire middleware into hooks:do (exec-mode only)
...
**Step 3: Implement default middleware + integration**
...
For function, registered, and exec-mode script implementations:
1) Run `_hooks:capture:run` with the implementation command.  
2) Call middleware:  
`"$middleware_fn" "$hook_name" "$exit_code" "$capture_var" -- "$@"`  
3) Use middleware exit code as `last_exit_code`.

For source-mode scripts, keep current behavior and skip middleware.
...
### Task 5: Expand middleware tests (default + custom behavior)
...
Run: `shellspec spec/hooks_spec.sh -f d`  
Expected: FAIL if middleware return values are not applied.

**Step 3: Ensure middleware return code is used**
...
git commit -m "test: cover middleware exit code overrides"
```

### Task 6: Update public docs with middleware contract
...
### Task 7: Add middleware demo in demos/ci-mode
...
### Task 8: Update middleware plan doc (optional sync)
...
git commit -m "docs: align middleware plan with v3 details"
```
