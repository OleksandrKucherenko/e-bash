...
bin/logs.sh capture \
  --service "api=node server.js" \
  --service "db=docker logs -f my-db"
...
Each registered service is a `tag=command` pair. `logs.sh capture` launches every
...
```bash
bin/logs.sh capture --service "web=./serve" --service "worker=./work"
...
```bash
bin/logs.sh capture --highlight "TODO:cyan,DEPRECATED:yellow" --service "app=./app"
```
...
Beyond repeatable `--service` flags, services can be declared in a file of
...
present. CLI `--service` flags win over config entries with the same tag.
...
bin/logs.sh capture \
  --service "web=npm run dev" \
  --service "worker=npm run worker" \
  --service "db=docker logs -f pg" \
  --highlight "ERROR:lred,WARN:yellow,SLOW:purple"

# Capture silently to a chosen directory with timestamps, then search later:
bin/logs.sh capture --no-view --timestamps --out /tmp/run \
  --service "job=./batch.sh"
bin/logs.sh search --base /tmp/run --grep failed

# Separated tmux view (one pane per service):
bin/logs.sh capture --separated --service "a=./a" --service "b=./b"
